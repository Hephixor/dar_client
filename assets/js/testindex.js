    ...
    if (sessionStorage.getItem('color') !== null) {
        var colorarray = JSON.parse(sessionStorage.getItem('color'));
        stylechange(colorarray);
    } else {
        var colorarray = [52, 152, 219]; // 52.152.219 = #3498DB
        sessionStorage.setItem('color', JSON.stringify(colorarray));
        stylechange(colorarray);
    }

    // Helpers
    function setName(name) {
        $.trim(name) === '' || $.trim(name) === null ? name = 'TestJS' : name = name;
        $('h1').text(name);
    ...
    function setModal(mode, $ctx) {
        ...
        switch (mode) {
            ...
            $mod.find('#new-user').val($ctx.find('.name').text()).addClass('used');
            break;
        }
        $mod.fadeIn();
    ...
    $('#colorpick').on('mousedown', function(eventDown) {
        ...
        if (eventDown.button === 0) {
            ...
            sessionStorage.setItem('color', JSON.stringify(imgData));
            stylechange(imgData);
        }
    });
    ...
    $('.list-account > .list').on('click', 'li', function() {
        ...
        $(this).parent().find('.hangouts.context').remove();
        $(this).addClass('active');

        var $TARGET = $(this);
        
        if (!$(this).next().hasClass('context')) {
            var $ctx = $('<li class="hangouts context"><i class="mdi mdi-pencil"></i><i class="mdi mdi-delete"></i></li>');
            ...
            $ctx.on('click', '.mdi-pencil', function() {
                ...
                $('#contact-modal').one('click', '.btn.save', function() {
                   ...
                   var idUserB  = $TARGET.find('.idUserB').text();
                     //Requete changement pseudo dans BDD contacts
                //Creation requet XMLHttp
                var xhr = null; 
                ...
                //Appel a messengerModifyContactPseudo.php et passage des paramètres
                ...
                xhr.open("POST","./php/messengerModifyContactPseudo.php",true);
                xhr.setRequestHeader('Content-Type','application/x-www-form-urlencoded');

                // arguments
                xhr.send("idUserB="+idUserB+"&pseudo="+$('#new-user').val());
                closeModal();
            });
});
    ...
    $('#add-contact-floater').on('click', function() {
        ...
        setModal('add');
        $('#contact-modal').one('click', '.btn.save', function() {
            $('.list-account > .list').prepend('<li class=\"hangouts\" ><img src="./images/members/unknown.png"><span class="name">' + $('#new-user').val() + '</span><i class="mdi mdi-menu-down"></i></li>');
            closeModal();
        });
    }
});